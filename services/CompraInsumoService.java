...
public class CompraInsumoService {
    ...
    public CompraInsumoDTO save(CompraInsumoDTO compraInsumoDTO) {
        ...
        // Validar y asociar insumos en los detalles
        if (compraInsumo.getDetalles() != null && compraInsumoDTO.getDetalles() != null) {
            for (int i = 0; i < compraInsumo.getDetalles().size(); i++) {
                ...
                if (detalleDTO.getArticuloInsumo() != null && detalleDTO.getArticuloInsumo().getId() != null) {
                    ArticuloInsumo articuloInsumo = articuloInsumoRepository.findById(detalleDTO.getArticuloInsumo().getId())
                            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                                    "Artículo insumo no encontrado con ID: " + detalleDTO.getArticuloInsumo().getId()));
