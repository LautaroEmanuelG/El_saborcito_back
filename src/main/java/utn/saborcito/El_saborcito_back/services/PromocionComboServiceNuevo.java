...
public class PromocionComboService {
    ...
    private boolean estaVigente(Promocion promocion, LocalDate fecha, LocalTime hora) {
        if (promocion.getFechaDesde().isAfter(fecha) ||
                promocion.getFechaHasta().isBefore(fecha)) {
            return false;
        }
     ...
     */
    private void crearDetallesPedidoPromocion(Pedido pedido, Promocion promocion, int veces, List<DetallePedido> detalles) {
